...
const generate = ({ foundations, semantic, contrastPairs }) => {
    ...
    const typographyClasses = Object.keys(semantic.typography)
        .map(
            name => `.type-${name} {
    font-family: var(--type-${cssName(name)}-family);
    font-size: var(--type-${cssName(name)}-size);
    font-weight: var(--type-${cssName(name)}-weight);
    line-height: var(--type-${cssName(name)}-line-height);
    letter-spacing: var(--type-${cssName(name)}-letter-spacing);
}`,
        )
        .join('\n\n')
...
`
    ...
    const typographySizes = Object.fromEntries(
        Object.keys(semantic.typography).map(name => [
            name,
            [
                `var(--type-${cssName(name)}-size)`,
                {
                    lineHeight: `var(--type-${cssName(name)}-line-height)`,
                    letterSpacing: `var(--type-${cssName(name)}-letter-spacing)`,
                },
            ],
        ]),
    )
...
const cssName: (tokenPath: string) => string = ${cssName.toString()}
...
export const foundationToken = (identifier: string): string =>
    \`var(--foundation-\${cssName(identifier)})\`
`
    const electronTs = `// Generated by tools/design-tokens.cjs. Do not edit.
export const nativeWindowBackgroundColor = '${colorToHex(resolvedSemanticColors['background.canvas'])}'
`
...
const checkStyles = (severity = 'warn') => {
    ...
    const visit = directory => {
        for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
            const file = path.join(directory, entry.name)
            if (entry.isDirectory()) visit(file)
            else if (/\.(?:css|ts)$/.test(entry.name) && !entry.name.endsWith('.spec.ts')) {
                diagnostics.push(
                    ...scanStyleSource({
