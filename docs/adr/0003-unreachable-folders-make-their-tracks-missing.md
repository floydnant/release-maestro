# An unreachable library folder makes its tracks missing, it does not fail the scan

When a configured library folder cannot be reached — an unplugged drive, an unmounted share, a
deleted directory — the scan drops it from the walk and continues over the folders it can reach.
Everything under the unreachable folder is then reconciled as missing, exactly like any other file
discovery did not see. Unreachable folders are reported on the scan status (`unavailableFolders`),
not as an error.
...
This includes the case where _no_ configured folder is reachable: that scan walks nothing, discovers
...
- **Nothing in the UI blocks on folder availability.** Both folder pickers still call
  `library:validate-folders`, but purely to display status — an unreachable folder is annotated, a
