...
const TAILWIND_STYLESHEET = path.join(FIXTURES, 'tailwind.css')
 ...
 */
const authorities = {
    tailwindStylesheet: TAILWIND_STYLESHEET,
...
templateTester.run('valid-template-classnames', templateRule, {
    valid: [
        ...
        template(
            'A2 design token reached through a CSS variable',
            '<div class="bg-[color-mix(in_srgb,var(--color-background-surface)_40%,transparent)]"></div>',
        ),
        template('A2 theme variable', '<div class="bg-[var(--color-status-info-background)]"></div>'),
    ...
    invalid: [
        ...
        template(
            'R6 unknown design token inside an arbitrary value',
            '<div class="bg-[color-mix(in_srgb,var(--color-background-surfac)_40%,transparent)]"></div>',
            {
                errors: [
                    { messageId: 'unknownThemeVariable', data: { variable: '--color-background-surfac' } },
                ],
            },
        ),
        template(
                ...
                errors: [
                    {
                        ...
                        data: { variable: '--foundation-color-neutral-500' },
                    },
                ],
            },
        ),
        template(
            'R6 deprecated theme function',
            '<div class="bg-[theme(colors.status.info-background)]"></div>',
            {
                errors: [{ messageId: 'deprecatedThemeFunction' }],
            },
        ),
...
typescriptTester.run('valid-host-classnames', hostRule, {
    ...
    invalid: [
        ...
        host(
            'R6 unknown design token in host metadata',
            "@Component({ host: { class: 'text-[var(--color-content-mute)]' } }) class C {}",
            {
                errors: [{ messageId: 'unknownThemeVariable', data: { variable: '--color-content-mute' } }],
            },
        ),
...
describe('S1 nearest-value suggestions in the utility’s own scale', () => {
    const candidates = tailwindClassList(TAILWIND_STYLESHEET)
