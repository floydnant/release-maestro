...
Before handing back, delegate a review to a sub-agent running on a model from a **different vendor** than the one that did the work. Self-review is not a substitute; the point is fresh eyes you cannot bring yourself. The sub-agent reads the audit trail and the run's transcript, then flags what the user should pay attention to. Not a redo of the work, a scan for what's suboptimal or risky.

If the runtime can delegate but not to another vendor, use the most different model it does offer and name it. If it cannot delegate at all, say so in the Attention section instead of substituting self-review — an unreviewed trail should look unreviewed.
